...
$num_rows = $henvendelser_result->num_rows;
?>
                ...
                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                    <td class="px-6 py-4">
                        <div class="flex items-center">
                            <?php
                            $henvendelse_id_stmt = $conn->prepare("SELECT id FROM henvendelser WHERE bruker_id = ? AND statuser_id = ?");
                            $henvendelse_id_stmt->bind_param("ii", $user_id, $row['statuser_id']);
...
<div class="relative overflow-x-auto sm:rounded-lg">
    <table class="w-full text-sm text-left rtl:text-right text-gray-500">
        <tbody>
            <?php
            // Resetting the pointer of the result set
            mysqli_data_seek($henvendelser_result, 0);
...
<?php else: ?>
<div class="flex flex-col items-center justify-center h-screen mx-8 pb-40 space-y-4">
    <h2 class="text-xl text-gray-500">Dine henvendelser vil dukke opp her...</h2>
    <a href="../index.php" class="bg-red-600 hover:bg-red-700 text-white py-2 px-4 rounded">
        Gå tilbake
    </a>
</div>
<?php endif; ?>
