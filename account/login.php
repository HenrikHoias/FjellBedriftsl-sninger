...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['login'])) {
        ...
        $stmt = $conn->prepare("SELECT id, pwd FROM bruker WHERE e_post = ?");
        $stmt->bind_param("s", $email);
        $stmt->execute();
        $result = $stmt->get_result();
        if ($result->num_rows == 1) {
            $row = $result->fetch_assoc();
            if (hash('sha256', $password) === $row['pwd']) {
                $_SESSION['user_id'] = $row['id'];
                header("Location: ../index.php");
                exit();
            } else {
                $login_err = "Feil passord.";
            }
        } else {
            $login_err = "Brukeren eksisterer ikke.";
        }
    }
}
?>
...
<head>
    ...
    <title>Logg inn | Fjell bedriftsløsninger</title>
        ...
        <div class="bg-red-600 dark:bg-gray-900 flex justify-center rounded-t-lg">
            ...
            <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
                ...
                <form class="space-y-4 md:space-y-6" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                    ...
                    <button type="submit" name="login" class="w-full text-white bg-gray-600 hover:bg-gray-700 focus:ring-4 focus:outline-none focus:ring-gray-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-gray-600 dark:hover:bg-gray-700 dark:focus:ring-gray-800">Logg inn</button>
                    <p class="text-sm font-light text-gray-500 dark:text-gray-400">
                        Har du ikke en konto? <a href="register.php" class="font-medium text-gray-600 hover:underline dark:text-gray-500">Registrer deg</a>
                    </p>
                </form>
            </div>
        </div>
